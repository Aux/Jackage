...
public static class JackboxLocator
{
    public static IEnumerable<string> GetJackboxInstalls()
    {
    ...
    private static IEnumerable<string> FindSteamInstalls()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            foreach (var value in vdf.Value.Children())
            {
                ...
                var jackboxPaths = allGames.Where(x => Path.GetFileName(x).StartsWith(JackboxConstants.PartyPackName));
                if (jackboxPaths.Any())
    ...
    private static string FindEpicLocation()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
